...
from storage.local_storage import upload_file
...
def extract_table(
    ...
    while True:
        ...
        f"→ {file_path.name} "
        f"→ {stored_file}"
        f"→ storage/{STORAGE_PREFIX}/{TABLE_NAME.split('.')[-1]}/"
)
...
You now have a validated, dynamic Python ingestion framework:
...
Python
    ...
    ├── Source vs extracted validation
    └── Storage abstraction
    │
    ▼
...
Total records = 19,131
Difference = 0
Your validation is especially important

You have verified:

Source records      = 19,131
Stored JSON records = 19,131
Difference          = 0

That means we can confidently move forward without worrying that our extractor is silently losing records.
