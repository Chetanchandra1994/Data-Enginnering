...
#}
{% macro generate_unexpected_values_test_ddl(warehouse,schedule,values,table,key,table_name,etl_event_detail_table,governance_db) %}
    ...
    {% for column, value in values.columns.items() %}        
        ...
        {% set create_task_sql %}
    SELECT 
         --md5(cast(CONCAT(CAST(OBJECT_CONSTRUCT(*) AS TEXT),CAST({{column}} as TEXT)) as TEXT)) as ETL_EVENT_DETAIL_SK
         md5(cast(CONCAT(CAST(UPPER('{{table}}') AS TEXT), CAST(UPPER('{{column}}') AS TEXT), CAST(UPPER({{key}}) as TEXT), CAST('2000' AS TEXT)) as TEXT)) as ETL_EVENT_DETAIL_SK
        ...
        ,2000 AS ETL_EVENT_DESCRIPTION_CODE
...
UNION ALL
    ...
    WHERE 
        S1.ETL_EVENT_DETAIL_SK is null AND T1.ETL_EVENT_STATUS <> 'INACTIVE' AND UPPER(T1.TABLE_KEY) = UPPER('{{table}}') AND UPPER(T1.FIELD_KEY) = UPPER('{{key}}') AND T1.ETL_EVENT_DESCRIPTION_CODE = 2000
