...
with DAG(
    dag_id='spm.fullload.cmg_outbound_databases',
    ...
    progress_source = ProgressSourceConnector(
        conn_id='acct',
        database='acct',
        sql='SELECT * FROM PUB."cmg_outbound_databases"',
        query_mode='default'
    )
    ...
    gcs_target = GCSTargetConnector(
        ...
        table_name='cmg_outbound_databases',
        gcs_path='raw/SPM_CA'
    )

    # 3. Use the Operator to run the task
    extract_data = ExtractionOperator(
        task_id='extract_cmg_outbound_databases',
